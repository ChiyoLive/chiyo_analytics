...
- 服务器上已安装 **Python 3**（用于执行安装管理脚本，一般来说现代 linux 发行版都会自带一个系统级 python3，用那个就行。我们提供一个自包含的 python zipapp 脚本，不需要安装任何依赖）。
...
### 显式配置许可同意状态
一般来说，你不需要显式调用 `setConsent` 来配置用户许可状态，而应该是通过调用 `ui.banner.render()` 渲染隐私合规 Banner 来获取用户同意。

这个接口是留给已经在使用自己的 Cookie Banner 或有自己的一套隐私合规处理系统的人的。当你需要手动设置 cyanly 的用户同意事项的时候，你可以使用 `setConsent` API 进行设置：
- **MPA SDK**：
  ```javascript
